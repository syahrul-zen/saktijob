...
<div class="row min-vh-100">
    <div class="col-lg-5 col-12 d-flex align-items-center justify-content-center">
        <div id="auth-left" style="max-width:420px;width:100%;background:#fff;border-radius:16px;padding:32px;box-shadow:0 12px 30px rgba(11,36,71,0.12)">
            <h1 class="auth-title fs-4 text-primary" style="margin-bottom:8px">
                <?= (strtolower(service('request')->getGet('role') ?? '') === 'perusahaan')
                    ? 'Daftar sebagai Perusahaan / Usaha'
                    : ((strtolower(service('request')->getGet('role') ?? '') === 'user')
            ...
            <form action="<?= site_url('register') ?>" method="post">
                <?= csrf_field() ?>
                <input type="hidden" name="role" value="<?= esc(strtolower(service('request')->getGet('role') ?? 'user')) ?>">
                <div class="form-group position-relative has-icon-left mb-4">
                    <input type="email" name="email" class="form-control" placeholder="Email" value="<?= esc(old('email')) ?>">
                ...
                <div class="form-group position-relative has-icon-left mb-4">
                    <input type="password" name="password_confirm" class="form-control" placeholder="Konfirmasi Kata Sandi">
                    <div class="form-control-icon">
                        <i class="bi bi-shield-lock"></i>
                    </div>
                </div>
            ...
            <div class="text-center mt-5 text-lg">
                <p class="text-gray-600 fs-6">
                    <a href="<?= base_url('login') ?>" class="font-bold fs-6 text-primary">
    ...
    <div class="col-lg-7 d-none d-lg-block d-flex align-items-center justify-content-center">
        <div id="auth-right" class="d-flex align-items-center justify-content-center" style="height:100vh;border-radius:25px 0 0 25px;overflow:hidden;position:relative;background: linear-gradient(145deg,rgba(11, 36, 71, 1) 0%, rgba(30, 136, 229, 1) 50%, rgba(242, 140, 40, 1) 100%);">
            <img src="<?= base_url('assets/admindash/assets/compiled/jpg/building.jpg') ?>" alt="Background" style="max-width:100%;max-height:100%;object-fit:cover;border-radius:70px 0 70px 0;">
